# For http://www.pythonchallenge.com/pc/def/equality.html L3
# Using regular expression

import re

def findletter(text):
    filter = re.compile("[a-z][A-Z][A-Z][A-Z][a-z][A-Z][A-Z][A-Z][a-z]")
    result = re.findall(filter, text)
    key = ""
    for word in result:
        key += word[len(word)/2]
    return key
